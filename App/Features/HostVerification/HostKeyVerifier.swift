...
actor HostKeyVerifier {
    ...
    private var pendingMismatches: [String: PendingHostKeyMismatch] = [:]
    ...
    func verify(
        ...
        let storedForHost = try await store.lookupAll(hostname: hostname, port: port)
        ...
        pendingMismatches[Self.hostKey(hostname: hostname, port: port)] = PendingHostKeyMismatch(
            hostname: hostname,
            port: port,
            keyType: keyType,
            publicKeyData: publicKeyData,
    ...
    /// Returns and clears the mismatch recorded for a host (set when `verify`
    /// blocked a known host that presented an unrecognised key).
    func takePendingMismatch(hostname: String, port: UInt16) -> PendingHostKeyMismatch? {
        pendingMismatches.removeValue(forKey: Self.hostKey(hostname: hostname, port: port))
    }

    private static func hostKey(hostname: String, port: UInt16) -> String {
        "\(KnownHostsStore.canonicalHostname(hostname)):\(port)"
    }
